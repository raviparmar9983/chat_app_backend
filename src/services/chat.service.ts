...
export const accessChatService = async (loginUserId: string, userId: string) => {
    try {
        const isChat = await Chat.findOne({
            isGroupChat: false,
            users: {
                $all: [new mongoose.Types.ObjectId(loginUserId), new mongoose.Types.ObjectId(userId)]
            }
        });
        if (isChat) {
            return {
...
export const createGroupChatService = async (userId: string, groupChatData: any) => {
    try {
...
export const addUserTOGroupService = async (adminId: string, chatId: string, userId: string) => {
    try {
        const chat = await Chat.findOneAndUpdate({
            _id: chatId,
            groupAdmin: adminId,
...
export const deleteUserFromGroup = async (adminId: string, chatId: string, userId: string) => {
    try {
        const chat = await Chat.findOneAndUpdate({
            _id: chatId,
            groupAdmin: adminId,
